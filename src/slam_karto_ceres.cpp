...
class SlamKartoCeres {
 ...
 private:
  ...
  std::unique_ptr<tf::TransformBroadcaster> tfB_ = nullptr;
  ...
  std::unique_ptr<CeresSolver> solver_ = nullptr;
  ...
  bool got_map_ = false;
  int laser_count_ = 0;
  std::unique_ptr<boost::thread> transform_thread_ = nullptr;
...
SlamKartoCeres::SlamKartoCeres() {
  ...
  ss_ =
      node_.advertiseService("dynamic_map", &SlamKartoCeres::mapCallback, this);
  scan_filter_sub_ =
  ...
  scan_filter_ = std::make_unique<tf::MessageFilter<sensor_msgs::LaserScan>>(
      *scan_filter_sub_.get(), tf_, odom_frame_, 5);
  scan_filter_->registerCallback(
      boost::bind(&SlamKartoCeres::laserCallback, this, _1));
  ...
  transform_thread_ = std::make_unique<boost::thread>(boost::bind(
      &SlamKartoCeres::publishLoop, this, transform_publish_period));
  ...
  solver_ = std::make_unique<CeresSolver>();
...
karto::LaserRangeFinder* SlamKartoCeres::getLaser(
    const sensor_msgs::LaserScan::ConstPtr& scan) {
...
void SlamKartoCeres::laserCallback(
    const sensor_msgs::LaserScan::ConstPtr& scan) {
  laser_count_++;
...
bool SlamKartoCeres::addScan(karto::LaserRangeFinder* laser,
                             const sensor_msgs::LaserScan::ConstPtr& scan,
                             karto::Pose2* karto_pose) {
